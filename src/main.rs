use eframe::{egui, Frame, run_native};
use eframe::egui::{Color32, Context, FontId, vec2};
...
fn main(){
    ...
    run_native("todos.rs", native_options, Box::new(|cc| Box::new(Todos::new(cc)))).expect("TODO: panic message");
    ...
    fn default() -> Self {
        Todos{
            ...
            visuals: Visuals {
                ...
                bin_img_texture_handle: RetainedImage::from_image_bytes("bin", include_bytes!("bin.png")).
                    expect("reasons"),
    ...
    fn update(&mut self, ctx: &Context, frame: &mut Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.vertical_centered(|ui|{
                ...
                let text_edit = ui.add(egui::TextEdit::singleline(&mut self.new_todo)
            ...
            ui.horizontal(|ui|{
                ...
                if self.todos.len() == 1 && !self.todos[0].1{
                    ...
                    ui.label(RichText::new(format!("{} tasks left", self.todos.len())).size(NORMAL_FONT_SIZE));
                }

                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui|{
                    ui.add_space(HORIZONTAL_SPACING);
                    let completed = ui.add(egui::Button::new(RichText::new("Completed").size(NORMAL_FONT_SIZE)).rounding(BUTTON_ROUNDING).fill(self.visuals.completed_button_bg_color));
                    ui.add_space(SPACING_BETWEEN_BUTTONS);
                    let active = ui.add(egui::Button::new(RichText::new("Active").size(NORMAL_FONT_SIZE)).rounding(BUTTON_ROUNDING).fill(self.visuals.active_button_bg_color));
                    ui.add_space(SPACING_BETWEEN_BUTTONS);
                    let all = ui.add(egui::Button::new(RichText::new("All").size(NORMAL_FONT_SIZE)).rounding(BUTTON_ROUNDING).fill(self.visuals.all_button_bg_color));
                ...
                for (index, todo) in self.todos.iter_mut().enumerate(){
                    ...
                    ui.horizontal(|ui|{
                        ...
                        match self.filter{
                            Filter::Completed => {
                                if todo.1 {
                                    ui.add(egui::Checkbox::new(&mut todo.1, RichText::new(&todo.0).size(CHECKBOX_TEXT_FONT_SIZE)));
                                }
                            }
                            Filter::Active => {
                                if !todo.1{
                        ...
                        if (self.filter == Filter::Completed && todo.1) || (self.filter == Filter::Active && !todo.1) || (self.filter == Filter::All){
                            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui|{
                                ui.add_space(HORIZONTAL_SPACING);
                                let bin_image_button = ui.add(egui::ImageButton::new(self.visuals.bin_img_texture_handle.texture_id(ctx), IMAGE_DIMENSIONS));
